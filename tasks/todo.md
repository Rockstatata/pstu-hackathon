...
- [ ] `backend/` FastAPI app skeleton, settings, health endpoint — **activate existing `backend/venv`, never recreate**
- [ ] SQLAlchemy models: `users`, `accounts`, `journal_entries`, `transfers`, `idempotency_records`
- [ ] Alembic init + first migration
- [ ] Seed the system **issuance account**
- [ ] `web/` Next.js PWA skeleton, Tailwind, lucide-react, mobile-first shell (bottom tabs → sidebar at `lg`)
- [ ] CORS allowlist + `NEXT_PUBLIC_API_URL`; JWT sent as `Authorization: Bearer`, **not** a cookie

## Phase 0.5 — Prove the deploy path (30m, by 11:50) — *do this with a skeleton, not at 14:00*

- [ ] Azure VM (B-series, student plan), Docker + Compose installed
- [ ] Enable the `*.cloudapp.azure.com` DNS name on the VM's public IP
...
- [ ] Register: phone + name + bcrypt-hashed PIN
- [ ] Issue ৳100,000 as a two-legged journal entry from the issuance account
- [ ] Login, JWT, auth dependency
- [ ] Screens A1, A2

## Phase 2 — Transfer engine (60m, by 12:50) — *the core; do not rush this*

- [ ] Money as integer **poisha** end to end
- [ ] `POST /transfers` with `Idempotency-Key` header
- [ ] Idempotency record inserted **in the same transaction**; unique violation is the concurrency guard
- [ ] Replay returns the stored original response; different body with same key returns 409
- [ ] Collect all touched accounts, **sort by account ID ascending**, `SELECT ... FOR UPDATE` in order
- [ ] Set `lock_timeout`; no deadlock-retry loop (ADR-0003)
- [ ] Write journal legs summing to zero + update cached balances in one transaction
- [ ] Balance check inside the lock; reject overspend
- [ ] Transfer Policy module: amount > 0, ≤ ৳100,000/txn, ≤ ৳200,000/day
- [ ] Recipient lookup endpoint (name + masked phone)
...
- [ ] `GET /admin/integrity` computing all five assertions live, no caching:
  - [ ] global journal sum = 0
  - [ ] per-account journal sum = cached balance
  - [ ] zero negative balances
  - [ ] every transfer has ≥2 legs summing to zero
  - [ ] issuance account = −(sum of user balances)
- [ ] Counters: completed transfers, idempotent replays, rejected overspends, step-ups
- [ ] Replica health across the 3 API instances
- [ ] Screen I1

## Phase 6 — Concurrency proof (30m, by 14:50)

- [ ] k6: duplicate-key storm ×50 → exactly 1 transfer
- [ ] k6: 20 concurrent sends of ৳100 from ৳1,000 → exactly 10 succeed
- [ ] k6: A↔B bidirectional + group pressure → zero deadlocks
- [ ] k6: sustained load across replicas → integrity green after
- [ ] `docker kill` a replica mid-load → no money lost
...
- [ ] Atomic multi-leg transfer, all-or-nothing (ADR-0002)
- [ ] Same sorted lock acquisition over N+1 accounts
...
- [ ] Deterministic rule table in the policy module: ≥ ৳25,000, first-time recipient, ≥5 in 10 min
- [ ] Persist the risk decision + reason on the transfer
