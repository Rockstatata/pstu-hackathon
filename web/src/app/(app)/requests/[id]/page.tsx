...
export default function RequestDetailPage() {
  ...
  async function pay() {
    ...
    setError(null);
    try {
      const transfer = await api.payMoneyRequest(request.id, idempotencyKey);
  ...
  if (!request) return <div className="mx-auto max-w-md"><FixtureNotice /><EmptyState icon={ReceiptText} title="Request unavailable" detail={error ?? "This request is no longer available."} action={{ label: "Back to requests", onClick: () => router.push("/requests") }} /></div>;
  ...
  return (
    <div className="mx-auto max-w-md pb-10">
      ...
      <div className="flex items-start justify-between gap-4"><div><h1 className="text-[24px] font-semibold leading-8">Money request</h1><p className="mt-1 text-[15px] text-text-secondary">From {request.counterpartyName}</p></div><StatusBadge status={request.status} /></div>
      ...
      {reviewing && recipient && <ConfirmationSheet amountMinor={request.amountMinor} recipient={recipient} note={request.reason} title="Pay money request" eyebrow="Check who you are paying" confirming={working} disabled={!online} error={error} onCancel={() => !working && setReviewing(false)} onConfirm={pay} />}
    </div>
  );
}
