...
export default function CreateScheduledTransferPage() {
  ...
  useEffect(() => {
    ...
    api.account()
      ...
      .catch(() => active && setError("Your balance is unavailable. Reconnect before scheduling money."));
    return () => { active = false; };
  }, []);
  ...
  useEffect(() => {
    ...
    api.recipientPreview(phone)
      ...
      .catch((cause) => active && setLookup({
        phone,
        recipient: null,
        error: cause instanceof ApiError ? cause.sentence : "We could not find that recipient.",
      }));
    return () => { active = false; };
  }, [phone]);
  ...
  return (
    <div className="mx-auto max-w-md pb-24">
      ...
      <Link href="/scheduled" className="mb-3 inline-flex min-h-11 items-center gap-1 text-[13px] font-semibold text-primary-text hover:underline"><ArrowLeft aria-hidden className="size-4" />Scheduled Transfers</Link>
      <h1 className="text-[24px] font-semibold leading-8">Schedule a Transfer</h1>
      <p className="mt-1 text-[15px] leading-6 text-text-secondary">Your PIN authorizes the future instruction. Balance, limits, and recipient checks run again when it is due.</p>
      <div className="mt-7 space-y-5">
        <PhoneInput value={phone} onChange={setPhone} error={phoneError} label="Recipient phone number" disabled={!online} autoFocus />
        ...
        {balanceWarning && <p className="-mt-3 text-[13px] leading-5 text-warning-text">Your current balance is lower than this amount. You can still authorize the instruction; the final balance check happens when it is due.</p>}
        <div>
          <label htmlFor="execute-at" className="text-[13px] font-medium text-text-secondary">Date and time</label>
        ...
        <div>
          <label htmlFor="schedule-note" className="text-[13px] font-medium text-text-secondary">Note <span className="font-normal">(optional)</span></label>
          <textarea id="schedule-note" value={note} onChange={(event) => setNote(event.target.value.slice(0, 140))} rows={3} disabled={!online} className="mt-1.5 w-full resize-none rounded-md border border-control bg-surface px-3 py-2.5 text-[15px] text-text outline-none focus:border-primary disabled:opacity-55" placeholder="What is this for?" />
        </div>
      </div>
      ...
      <div className="safe-bottom fixed inset-x-0 bottom-[calc(56px+env(safe-area-inset-bottom))] z-20 border-t border-divider bg-bg/95 px-4 py-3 backdrop-blur lg:bottom-0 lg:left-[15rem]">
        <div className="mx-auto max-w-md"><Button full onClick={openReview} disabled={!online || !recipient || amountMinor === null || amountMinor <= 0 || Boolean(amountError) || Boolean(scheduleError)}>Review schedule <ChevronRight aria-hidden className="size-4" /></Button></div>
      </div>
      {reviewing && recipient && amountMinor !== null && (
        <ConfirmationSheet amountMinor={amountMinor} recipient={recipient} note={note.trim()} title="Schedule Transfer" eyebrow="Check the future instruction" confirmLabel="Authorize schedule" confirming={submitting} disabled={!online} error={error} onCancel={() => !submitting && setReviewing(false)} onConfirm={() => void schedule()}>
          <p className="mt-4 flex items-start gap-2 text-[13px] leading-5 text-text-secondary"><CalendarClock aria-hidden className="mt-0.5 size-4 shrink-0 text-primary-text" />Will be attempted on {new Intl.DateTimeFormat("en-BD", { dateStyle: "medium", timeStyle: "short" }).format(scheduleDate)}. This does not reserve money.</p>
        </ConfirmationSheet>
      )}
