...
export default function GroupSendPage() {
  ...
  function addRecipient() {
    ...
    if (recipients.some((item) => item.userId === candidate.userId)) { setError("That recipient is already in this group."); return; }
  ...
  async function confirm() {
    ...
    setSubmitting(true);
    try {
      const transfer = await api.createGroupTransfer({ recipients: recipients.map((recipient) => ({ recipientUserId: recipient.userId, amountMinor: perPersonMinor })), note: note.trim() || undefined }, idempotencyKey);
  ...
  return (
    <div className="mx-auto max-w-md pb-24">
      ...
      {reviewing && recipients.length > 1 && totalMinor !== null && <ConfirmationSheet amountMinor={totalMinor} recipient={recipients[0]} recipients={recipients} note={note.trim()} confirming={submitting} disabled={offline} error={error} onCancel={() => !submitting && setReviewing(false)} onConfirm={confirm} />}
    </div>
  );
}
